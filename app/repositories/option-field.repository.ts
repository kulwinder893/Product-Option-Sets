...
export class OptionFieldRepository {
      ...
      while (pending.length > 0 && guard-- > 0) {
        ...
        for (const field of pending) {
          const parentId = field.parentId
            ? (idMap[field.parentId] ?? (isTempId(field.parentId) ? null : field.parentId))
            : null;
          ...
          if (isTempId(field.id)) {
            ...
            await this.syncChoices(tx, created.id, field, idMap, true);
          } else {
            await tx.optionField.update({
              where: { id: field.id },
              data: { ...columns, parentId },
            });
            await this.syncChoices(tx, field.id, field, idMap, false);
          }
        }
  ...
  private async syncChoices(
    ...
    if (!isNewField) {
      const keptChoiceIds = field.choices
        .filter((choice) => !isTempId(choice.id))
        .map((choice) => choice.id);

      await tx.fieldChoice.deleteMany({
        where: { fieldId, id: { notIn: keptChoiceIds.length ? keptChoiceIds : ["__none__"] } },
      });
    }
    ...
    for (const choice of field.choices) {
      ...
      if (isTempId(choice.id)) {
        ...
        const existing = await tx.fieldChoice.findUnique({
          where: { id: choice.id },
          select: { id: true },
        });
        if (existing) {
          await tx.fieldChoice.update({ where: { id: choice.id }, data: columns });
        } else {
          const created = await tx.fieldChoice.create({
            data: { ...columns, fieldId },
          });
